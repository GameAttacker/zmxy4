...
function JIANCE() {
  ...
  const _0x12e684 = _0x189894(this, function () {
    // 1. 还原出允许的域名：cdn.jsdelivr.net
const _0x5dd919 = new RegExp("[zfLpixgrjlSESiRoqNegWeMgSEMTKmXsXavgQGm]", "g");
// 这里将混淆字符串替换为可以还原出 cdn.jsdelivr.net 的文本
const _0x397bd7 = "czdfLpixn.grj1l5.jSE84S.52isRoqNegWeMdeMgSEMTKmlisvXavgQGme0r.n0et".replace(_0x5dd919, "").split(";");

// 2. 还原出跳转目标：about:blank
...
// ... （中间保持原样：遍历 window 动态获取 document 和 location 的逻辑） ...

// 3. 校验逻辑
let _0x56e35a = false;
for (let _0x318da8 = 0; _0x318da8 < _0x397bd7.length; _0x318da8++) {
  const _0x40a7d0 = _0x397bd7[_0x318da8];
  // 核心修改：原代码使用字符串尾部匹配（indexOf），这样就能匹配包含 cdn.jsdelivr.net 的域名
  const _0x5b163f = _0x3a4c81.length - _0x40a7d0.length;
  const _0x4e0382 = _0x3a4c81.indexOf(_0x40a7d0, _0x5b163f);
  if (_0x4e0382 !== -1 && _0x4e0382 === _0x5b163f) {
    _0x56e35a = true;
  }
}
