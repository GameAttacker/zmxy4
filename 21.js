// 在代码第一行加入这句，防止由于找不到变量而崩溃
window.Query = window.Query || true; 
window.h5gg = window.h5gg || {}; // 同理，顺便保护一下 h5gg
const AppState = {
...
function JIANCE() {
  ...
  const _0x12e684 = _0x189894(this, function () {
  {
    ...
    // 2. 核心修改：此处的混淆字符串经过上面的正则剔除后，还原出来的结果正是 "cdn.jsdelivr.net"
    ...
    // 4. 如果当前运行环境不是 cdn.jsdelivr.net，强行切到空白页 about:blank
    if (!_0x56e35a) {
      {
...
if (_0x3ee804 === "fangfeng") {
      ...
      // 2. 修复：用一个匿名的箭头函数 () => {} 包裹，确保 JIANCE() 在 1 秒后才真正执行
      setTimeout(() => {
        const _0x308e95 = JIANCE(); // 此时才会真正调用并进入检测
        if (_0x308e95) {
