if (typeof Query === "undefined") { window.Query = true; }
...
function JIANCE() {
  ...
  // 1. 环境严格检查：如果没有 H5GG 修改器环境，直接返回 false（不让前端跟着假嗨）
  if (typeof h5gg === "undefined") {
    console.log("未检测到 H5GG 环境，拒绝执行内存搜索");
    return false;
  }
  ...
  try {
    ...
    // 3. 设定精确的内存基址范围，发起第一次特征码搜索
    // 原始范围：0x100000000 到 0x300000000 之间搜索数字 1811
    h5gg.searchNumber('1811', 'I32', '0x100000000', '0x300000000');
    
    // 4. 邻近特征过滤（根据原代码的 conditions 矩阵还原）
    // 检查邻近偏移是否满足特定过检特征
    h5gg.filterNumber('159', 'I32'); // 对应条件 1

    // 获取最终过滤出来的有效地址总数
    let count = h5gg.getResultsCount();

    if (count > 0) {
      // 5. 真正开始批量改写内存（把原本报错的 executeDetection 替换为真正的写入循环）
      let results = h5gg.getResults(count); // 取出所有搜索到的内存地址
      
      for (let i = 0; i < results.length; i++) {
        let baseAddress = Number(results[i].address);
        
        // 依次改写原配置里要求的 7 处防封偏移数据
        h5gg.setValue(baseAddress + 4, 13568, 'I32');
        h5gg.setValue(baseAddress + 8, 0, 'I32');
        h5gg.setValue(baseAddress + 12, 0, 'I32');
        h5gg.setValue(baseAddress + 16, 0, 'I32');
        h5gg.setValue(baseAddress + 40, -353750312, 'I32');
        h5gg.setValue(baseAddress + 44, 202, 'I32');
        h5gg.setValue(baseAddress + 48, 0, 'I32');
      }
      ...
      // 如果方案 A 没搜到任何数据，转入备用防封方案 B
      if (typeof JIANCE1 === "function") {
        return JIANCE1();
      }
      return false;
    }
    ...
    console.error("真机内存改写发生崩溃保护:", error);
    return false;
  }
}
